...
userRouter.route('/profile/:p?')
            .get((request,response)=>{
            
               if(request.params.p == undefined){
                UserSchema.findOne({email:request.user.email})
                .then((data)=>{
                ...
                // .get((request,response)=>{
                  ...
                  UserSchema.findOne({email:request.user.email}).populate({path:"address.governorate_id address.city_id"})
                           .then((data)=>{
                               response.status(200).send(data)
                           })
                           .catch((err)=>{
            ...
            .put( upload.single('imageProfile'),(request,response)=>{
                ...
                UserSchema.updateOne({email:request.user.email},{
                    $set:{
                        ...
                        imageProfile: url + '/public/users/' + request.file.filename
                    }
                })
